...
export function addStreams(list: SniffResult[], tmdbId?: number, type?: string, season?: number, episode?: number) {
  ...
  for (const s of list) {
    ...
    if (idx >= 0) arr[idx] = { url: s.url, mime: s.mime || arr[idx].mime, referer: s.referer || arr[idx].referer, quality: s.quality || arr[idx].quality, headers: s.headers || arr[idx].headers, provider: s.provider || arr[idx].provider };
    else arr.push(s);
  }
